...
class TestEnsureHistory:
    ...
    def test_creates_new_entries(self):
        from bot_core.history import _chat_history, ensure_history
...
class TestChatHistory:
    ...
    def test_message_count_returns_zero_for_missing(self):
        from bot_core.history import get_message_count

        result = get_message_count(999999, 888888)
        assert result == 0
...
class TestChatHistoryIntegration:
    ...
    def test_set_active_char_and_get_it_back(self):
        from bot_core.history import set_active_char_key, get_active_char_key
